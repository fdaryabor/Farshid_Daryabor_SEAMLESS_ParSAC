...
ind['non-living_particulate_organic_carbon']=(1.0e6*(var['msi_ergom1_dd'])*var['h'])[:,:].sum(axis=1)/var['h'][:,:].sum(axis=1)
...
ind['net_primary_production']=((var['msi_ergom1_NPR'])*var['h'])[:,:].sum(axis=1)
...
ind['chl_in_5m']=((var['msi_ergom1_pp']+var['msi_ergom1_ff']+var['msi_ergom1_bb'])*var['h'])[:,70:].sum(axis=1)/var['h'][:,70:].sum(axis=1)

# "chl_max_in_10m_to_50m" expression="(msi_ergom1_pp+msi_ergom1_ff+msi_ergom1_bb)[:,:58].mean(axis=0).max(axis=0)" path="result.nc"/>
ind['chl_max_in_10m_to_50m']=(var['msi_ergom1_pp']+var['msi_ergom1_ff']+var['msi_ergom1_bb'])[:,:58].max(axis=1)
...
for t,date_time_obj in enumerate(date_list):
    ind['depth_of_chl_max_in_10m_to_50m'][t]=depth[t,int((var['msi_ergom1_pp']+var['msi_ergom1_ff']+var['msi_ergom1_bb'])[t,:58].argmax(axis=0))]



output = MODEL + '_L4' + '_ONEYEAR_INDICATORS.txt'
fid = open(output,'w')
header='DATE'
